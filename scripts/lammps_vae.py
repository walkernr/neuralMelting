...
if __name__ == '__main__':
    ...
    try:
        ...
        SKLD = np.load(PREF+'.%04d.%s.%s.%s.%02d.%04d.%.0e.%04d.zerr.kld.stdv.npy'
                       % (SNS, SCLR, OPT, LSS, LD, EP, LR, SEED))
        MXKLD = np.load(PREF+'.%04d.%s.%s.%s.%02d.%04d.%.0e.%04d.zerr.kld.max.npy'
                        % (SNS, SCLR, OPT, LSS, LD, EP, LR, SEED))
        MNKLD = np.load(PREF+'.%04d.%s.%s.%s.%02d.%04d.%.0e.%04d.zerr.kld.min.npy'
                        % (SNS, SCLR, OPT, LSS, LD, EP, LR, SEED))
        if VERBOSE:
    ...
    if VERBOSE:
        ...
        print(100*'-')
        print('error |'+ERRDISTN[0].size*' %1.2f' % tuple(ERRDISTN[1, :-1]))
        print('count |'+ERRDISTN[0].size*' %04d' % tuple(ERRDISTN[0, :].astype(np.int32)))
        print('error |'+ERRDISTP[0].size*' %1.2f' % tuple(ERRDISTP[1, 1:]))
        print('count |'+ERRDISTP[0].size*' %04d' % tuple(ERRDISTP[0, :].astype(np.int32)))
        print(100*'-')
    with open(OUTPREF+'.out', 'a') as out:
