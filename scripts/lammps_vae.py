...
if __name__ == '__main__':
    ...
    if VERBOSE:
        ...
        print(100*'-')
        print('error |'+ERRDISTN[0].size*' %1.40f' % tuple(ERRDISTN[1][:-1]))
        print('count |'+ERRDISTN[0].size*' %.2e' % tuple(ERRDISTN[0][:].astype(np.int32)))
        print('error |'+ERRDISTP[0].size*' %1.40f' % tuple(ERRDISTP[1][1:]))
